...
public class ObjectSetApiMockTest {
    ...
    public void objectSetApis_buildCorrectRequests() throws Exception {
        try (MockWebServer server = new MockWebServer()) {
            ...
            // 14) 设置 ObjectSet 标签：服务端成功契约为 204。
            PutObjectSetTaggingOutput putObjectSetTaggingOutput = client.putObjectSetTagging(
                    PutObjectSetTaggingInput.builder()
                            .bucket(bucket)
                            .objectSetName(objectSetName)
                            .tagSet(tagSet)
                            .build());
            Assert.assertEquals(putObjectSetTaggingOutput.getRequestInfo().getStatusCode(), 204);
            request = server.takeRequest();
            Assert.assertEquals(request.getMethod(), "PUT");
            url = request.getRequestUrl();
