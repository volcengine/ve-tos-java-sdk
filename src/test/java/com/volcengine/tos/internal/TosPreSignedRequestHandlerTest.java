...
public class TosPreSignedRequestHandlerTest {
    ...
    void preSignedURLObjectCURDTest() {
        ...
        header.put(TosHeader.HEADER_CONTENT_MD5, "XrY7u+Ae7tCTyyK7j1rNww==");
        input = new PreSignedURLInput().setHttpMethod(HttpMethod.PUT).setBucket(Consts.bucket)
                .setKey(key).setExpires(3600).setHeader(header).setSignedAllHeaders(true);
        url = handler.preSignedURL(input);
        try {
            ...
            Response resp2 = doReqWithHeaders(HttpMethod.PUT, url.getSignedUrl(), content, "hello world".length(), "", null);
            Assert.assertEquals(resp2.code(), HttpStatus.FORBIDDEN);
        } catch (IOException e) {
            testFailed(e);
        }
