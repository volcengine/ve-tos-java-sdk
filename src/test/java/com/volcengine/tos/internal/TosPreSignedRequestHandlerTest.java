...
public class TosPreSignedRequestHandlerTest {
    ...
    void preSignedPostSignatureTest() {
        ...
        try{
            ...
            Response response = doPostReq(Consts.bucket, key, Consts.endpoint, null, output, sampleData);
            ...
            Thread.sleep(3000);
            response = doPostReq(Consts.bucket, key, Consts.endpoint, null, output, sampleData);
    ...
    void preSignedPostSignatureWithConditionTest() {
        try{
            ...
            PreSignedPostSignatureOutput output = handler.preSignedPostSignature(input);
            Response response = doPostReq(Consts.bucket, key, Consts.endpoint, ACLType.ACL_PUBLIC_READ, output, sampleData);
            if (response.body() != null) {
                Consts.LOG.debug(StringUtils.toString(response.body().byteStream()));
            }
            Assert.assertEquals(response.code(), HttpStatus.NO_CONTENT);
            ...
            // invalid acl
            response = doPostReq(Consts.bucket, key, Consts.endpoint, ACLType.ACL_PRIVATE, output, sampleData);
            Assert.assertEquals(response.code(), HttpStatus.FORBIDDEN);
            response.close();

            // invalid key
            key = getUniqueObjectKey();
            response = doPostReq(Consts.bucket, key, Consts.endpoint, ACLType.ACL_PUBLIC_READ, output, sampleData);
            Assert.assertEquals(response.code(), HttpStatus.FORBIDDEN);
            response.close();
        } catch (Exception e) {
            testFailed(e);
    ...
    private Response doPostReq(String bucket, String key, String endpoint, ACLType acl, PreSignedPostSignatureOutput input, String data) throws IOException {
