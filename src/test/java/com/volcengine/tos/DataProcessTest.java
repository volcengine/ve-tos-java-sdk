...
public class DataProcessTest {
    ...
    public void DifferentContainerFormatTypeTest() {
        ...
        try {
            client.createBucket(new CreateBucketV2Input().setBucket(bucketName));
            List<ContainerFormatType> containerFormatTypes = Arrays.asList(ContainerFormatType.MP4, ContainerFormatType.TS, ContainerFormatType.HLS);
            for (ContainerFormatType containerFormatType : containerFormatTypes) {
                ...
                Container container = Container.builder()
                        ...
                        .build();
                if (containerFormatType == ContainerFormatType.HLS) {
                    container.setClipConfig(new ClipConfig().setDuration(10));
                }
    ...
    public void VideoConvertJobCrudTest(){
        ...
        try {
            ...
            TranscodeConfig transcodeConfig1 = TranscodeConfig.builder().transcode(transcodeConfig).build();
             ...
             Assert.assertEquals(createVideoConvertJobOutput.getCode(), "OK");
             Assert.assertEquals(createVideoConvertJobOutput.getMessage(), "Success");
             Assert.assertNotNull(createVideoConvertJobOutput.getJobId());
    ...
    @Test
    public void AnimationJobCrudTest() throws Exception {
        String dataProcessBucket = System.getenv("TEST_DATA_PROCESS_BUCKET");
        String dataProcessObject = System.getenv("TEST_DATA_PROCESS_OBJECT");
        if (dataProcessBucket == null || dataProcessObject == null) {
            throw new SkipException("TEST_DATA_PROCESS_BUCKET and TEST_DATA_PROCESS_OBJECT are required");
        }
        String bucketName = dataProcessBucket;
        String objectKey = dataProcessObject;
        String outputBucket = bucketName;
        String outputKey = "data-process-animation-output-" + System.currentTimeMillis() + ".gif";
        try {
            String asyncProcess = "x-tos-async-process=video/animation,w_0,h_400,ss_0,f_gif,fps_10,num_10,inter_200"
                    + "&x-tos-save-object=" + base64String(outputKey);
            CreateAsyncProcessTaskInput createInput = CreateAsyncProcessTaskInput.builder()
                    .bucket(bucketName)
                    .key(objectKey)
                    .asyncProcess(asyncProcess)
                    .build();
            CreateAsyncProcessTaskOutput createOutput = client.createAsyncProcessTask(createInput);
            Assert.assertNotNull(createOutput);
            Assert.assertEquals(createOutput.getRequestInfo().getStatusCode(), 200);
            Assert.assertNotNull(createOutput.getJobId());
            Consts.LOG.info("Async process task created with ID: {}", createOutput.getJobId());
            Consts.LOG.info("CreateAsyncProcessTask response: {}", Consts.JSON.writeValueAsString(createOutput));

            GetAnimationJobInput getInput = GetAnimationJobInput.builder()
                    .bucket(bucketName)
                    .key(objectKey)
                    .jobID(createOutput.getJobId())
                    .build();
            GetAnimationJobOutput getOutput = null;
            for (int i = 0; i < 6; i++) {
                try {
                    getOutput = client.getAnimationJob(getInput);
                    break;
                } catch (TosException e) {
                    if (e.getStatusCode() != 404 || i == 5) {
                        throw e;
                    }
                    Thread.sleep(1000);
                }
            ...
            Consts.LOG.info("Animation job got: {}", getOutput);
            Consts.LOG.info("GetAnimationJob response: {}", Consts.JSON.writeValueAsString(getOutput));
            Assert.assertEquals(getOutput.getRequestInfo().getStatusCode(), 200);
            Assert.assertEquals(getOutput.getJobID(), createOutput.getJobId());
            Assert.assertEquals(getOutput.getInput().getObject(), objectKey);
            Assert.assertEquals(getOutput.getOutput().getBucket(), outputBucket);
            Assert.assertEquals(getOutput.getOutput().getObject(), outputKey);
            Assert.assertNotNull(getOutput.getAnimationConfig());
            Assert.assertEquals(getOutput.getAnimationConfig().getContainer().getFormat(), ContainerFormatType.GIF);
            Assert.assertEquals(getOutput.getAnimationConfig().getAnimation().getWidth(), 0);
            Assert.assertEquals(getOutput.getAnimationConfig().getAnimation().getHeight(), 400);
            Assert.assertEquals(getOutput.getAnimationConfig().getAnimation().getFps(), 10);
            Assert.assertEquals(getOutput.getAnimationConfig().getAnimation().getFrameNum(), 10);
            Assert.assertEquals(getOutput.getAnimationConfig().getAnimation().getFrameInterval(), 200);
        } catch (TosException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        } finally {
            try {
                client.deleteObject(new DeleteObjectInput().setBucket(outputBucket).setKey(outputKey));
    ...
    @Test
    public void WatermarkTemplateCurdTest() {
        String bucketName = "java-sdk-watermark-" + System.currentTimeMillis();
        String templateId = null;
        try {
            client.createBucket(new CreateBucketV2Input().setBucket(bucketName));

            WatermarkText watermarkText = WatermarkText.builder()
                    .fontSize(50)
                    .fontType("wqy-microhei")
                    .fontColor("red")
                    .transparency(20)
                    .text("Test")
                    .build();
            Watermark watermark = Watermark.builder()
                    ...
                    .text(watermarkText)
                    .build();
            String templateName = "ut-watermark-" + System.currentTimeMillis();
            PutWatermarkTemplateOutput putOutput = client.putWatermarkTemplate(PutWatermarkTemplateInput.builder()
                    .bucket(bucketName)
                    .name(templateName)
                    .watermarkConfig(watermark)
                    .build());
            Assert.assertNotNull(putOutput);
            Assert.assertEquals(putOutput.getRequestInfo().getStatusCode(), 200);
            Assert.assertNotNull(putOutput.getId());
            templateId = putOutput.getId();

            GetWatermarkTemplateOutput getOutput = client.getWatermarkTemplate(GetWatermarkTemplateInput.builder()
                    .bucket(bucketName)
                    .id(templateId)
                    .build());
            Assert.assertEquals(getOutput.getRequestInfo().getStatusCode(), 200);
            Assert.assertEquals(getOutput.getTag(), "Watermark");
            Assert.assertEquals(getOutput.getId(), templateId);
            Assert.assertEquals(getOutput.getName(), templateName);
            Assert.assertNotNull(getOutput.getWatermarkConfig());
            Watermark outputWatermark = getOutput.getWatermarkConfig();
            Assert.assertEquals(outputWatermark.getType(), watermark.getType());
            Assert.assertEquals(outputWatermark.getPos(), watermark.getPos());
            Assert.assertEquals(outputWatermark.getLocMode(), watermark.getLocMode());
            Assert.assertEquals(outputWatermark.getDx(), watermark.getDx());
            Assert.assertEquals(outputWatermark.getDy(), watermark.getDy());
            Assert.assertEquals(outputWatermark.getStartTime(), watermark.getStartTime());
            Assert.assertEquals(outputWatermark.getEndTime(), watermark.getEndTime());
            Assert.assertNotNull(outputWatermark.getText());
            Assert.assertEquals(outputWatermark.getText().getFontSize(), watermarkText.getFontSize());
            Assert.assertEquals(outputWatermark.getText().getFontType(), watermarkText.getFontType());
            Assert.assertEquals(outputWatermark.getText().getFontColor(), watermarkText.getFontColor());
            Assert.assertEquals(outputWatermark.getText().getTransparency(), watermarkText.getTransparency());
            Assert.assertEquals(outputWatermark.getText().getText(), watermarkText.getText());

            ListWatermarkTemplatesOutput listOutput = client.listWatermarkTemplates(ListWatermarkTemplatesInput.builder()
                    .bucket(bucketName)
                    .build());
            Assert.assertEquals(listOutput.getRequestInfo().getStatusCode(), 200);
            boolean found = false;
            for (WatermarkTemplate template : listOutput.getWatermarkTemplates()) {
                if (templateId.equals(template.getId())) {
                    found = true;
                    Assert.assertEquals(template.getTag(), "Watermark");
            ...
            DeleteWatermarkTemplateOutput deleteOutput = client.deleteWatermarkTemplate(DeleteWatermarkTemplateInput.builder()
                    .bucket(bucketName)
                    .id(templateId)
                    .build());
            Assert.assertEquals(deleteOutput.getRequestInfo().getStatusCode(), 204);
            templateId = null;

            try {
                client.getWatermarkTemplate(GetWatermarkTemplateInput.builder()
                        .bucket(bucketName)
                        .id(putOutput.getId())
                        .build());
                Assert.fail("Get watermark template should fail after deletion");
            } catch (TosException e) {
                Assert.assertEquals(e.getStatusCode(), 404);
            }
        } catch (TosException e) {
            ...
            if (templateId != null) {
                try {
                    client.deleteWatermarkTemplate(DeleteWatermarkTemplateInput.builder()
                            .bucket(bucketName)
                            .id(templateId)
                            .build());
                } catch (TosException ignored) {
    ...
    @Test
    public void VideoConvertJobWithWatermarkTest() {
        String dataProcessBucket = System.getenv("TEST_DATA_PROCESS_BUCKET");
        String dataProcessObject = System.getenv("TEST_DATA_PROCESS_OBJECT");
        if (dataProcessBucket == null || dataProcessObject == null) {
            throw new SkipException("TEST_DATA_PROCESS_BUCKET and TEST_DATA_PROCESS_OBJECT are required");
        }
        String outputKey = "data-process-video-watermark-output-" + System.currentTimeMillis() + ".mp4";
        try {
            WatermarkText watermarkText = WatermarkText.builder()
                    .fontSize(50)
                    .fontType("wqy-microhei")
            ...
            Transcode transcode = Transcode.builder()
                    .container(Container.builder()
                            ...
                            .build())
                    .build();
            CreateVideoConvertJobOutput output = client.createVideoConvertJob(CreateVideoConvertJobInput.builder()
                    .bucket(dataProcessBucket)
                    .input(ConvertJobInput.builder().object(dataProcessObject).build())
                    .transcodeConfig(TranscodeConfig.builder()
                            .transcode(transcode)
                            .watermark(Arrays.asList(watermark))
                            .build())
                    .output(ConvertJobOutput.builder()
                            .region(Consts.region)
                            .bucket(dataProcessBucket)
                            .object(outputKey)
                            .build())
                    .build());
            Assert.assertEquals(output.getRequestInfo().getStatusCode(), 200);
            Assert.assertNotNull(output.getJobId());
            Consts.LOG.info("Video convert job with watermark created with ID: {}", output.getJobId());
        } catch (TosException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        } finally {
            try {
                client.deleteObject(new DeleteObjectInput().setBucket(dataProcessBucket).setKey(outputKey));
            } catch (TosException ignored) {
            }
        }
    }
