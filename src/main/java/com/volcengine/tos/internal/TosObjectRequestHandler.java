...
//        }
    ...
    private CopyObjectV2Output buildCopyObjectV2Output(TosResponse response) {
        ...
        try {
            output = PayloadConverter.parsePayload(rspMsg,
                    new TypeReference<CopyObjectV2Output>() {
                    });
        } catch (TosClientException e) {
            throw parseServerExceptionOrOriginal(rspMsg, response, e);
        }
        if (output == null || StringUtils.isEmpty(output.getEtag())) {
            throw buildServerExceptionFromBody(rspMsg, response);
        }
    ...
    private UploadPartCopyV2Output buildUploadPartCopyV2Output(UploadPartCopyV2Input input, TosResponse response) {
        String rspMsg = StringUtils.toString(response.getInputStream(), "copy result");
        UploadPartCopyOutputJson out;
        try {
            out = PayloadConverter.parsePayload(rspMsg,
                    new TypeReference<UploadPartCopyOutputJson>() {
                    });
        } catch (TosClientException e) {
            throw parseServerExceptionOrOriginal(rspMsg, response, e);
        }
        if (out == null || StringUtils.isEmpty(out.getEtag())) {
            throw buildServerExceptionFromBody(rspMsg, response);
        }
    ...
    private TosException parseServerExceptionOrOriginal(String rspMsg, TosResponse response, TosClientException originalException) {
        try {
            return buildServerExceptionFromBody(rspMsg, response);
        } catch (TosClientException ignored) {
            return originalException;
        }
    }

    private TosServerException buildServerExceptionFromBody(String rspMsg, TosResponse response) {
        ServerExceptionJson errMsg = PayloadConverter.parsePayload(rspMsg,
                new TypeReference<ServerExceptionJson>() {
                });
        if (errMsg == null || (StringUtils.isEmpty(errMsg.getCode()) && StringUtils.isEmpty(errMsg.getMessage()))) {
            throw new TosClientException("tos: invalid response body " + rspMsg, null);
        }
        return new TosServerException(response.getStatusCode(), errMsg.getCode(), errMsg.getMessage(),
    ...
    public FetchObjectOutput fetchObject(FetchObjectInput input) throws TosException {
        ...
        TosRequest req = this.factory.build(builder, HttpMethod.POST, new ByteArrayInputStream(marshalResult.getData()))
                .setContentLength(marshalResult.getData().length);
        return objectHandler.doRequest(req, HttpStatus.OK, this::buildFetchObjectOutput);
    }

    private FetchObjectOutput buildFetchObjectOutput(TosResponse response) {
        String rspMsg = StringUtils.toString(response.getInputStream(), "fetch result");
