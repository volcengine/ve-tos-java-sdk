...
public class UploadFileTaskHandler {
    ...
    public void initTask() {
        ...
        if (this.input.getCancelHook() != null && this.input.getCancelHook() instanceof UploadFileTaskCanceler) {
            ((UploadFileTaskCanceler) this.input.getCancelHook()).setHandler(this.handler).setTaskMan(this.taskMan).setBucket(checkpoint.getBucket()).setKey(checkpoint.getKey()).setUploadID(checkpoint.getUploadID()).setEnableCheckpoint(input.isEnableCheckpoint()).setCheckpointFilePath(input.getCheckpointFile());
        }
    ...
    public void dispatch() {
        for (int i = 0; i < this.checkpoint.getUploadPartInfos().size(); i++) {
            if (!this.checkpoint.getUploadPartInfos().get(i).isCompleted()) {
                taskMan.dispatch(new UploadFileTask(this.checkpoint, i, consumedBytes).setEnableCheckpoint(input.isEnableCheckpoint()).setCheckpointFile(input.getCheckpointFile()).setUploadEventListener(input.getUploadEventListener()).setOptions(input.getOptions()).setRateLimiter(input.getRateLimiter()).setDataTransferListener(input.getDataTransferListener()).setHandler(handler).setTrafficLimit(input.getTrafficLimit()));
            } else {
                UploadPartInfo partInfo = checkpoint.getUploadPartInfos().get(i);
                uploadedParts.add(new UploadedPartV2().setEtag(partInfo.getEtag()).setPartNumber(partInfo.getPartNumber()));
            }
        }
    }

    public UploadFileV2Output handle() {
        Util.postDataTransferStatus(this.input.getDataTransferListener(), new DataTransferStatus().setType(DataTransferType.DATA_TRANSFER_STARTED).setTotalBytes(checkpoint.getFileSize()).setConsumedBytes(this.consumedBytes.get()));
        this.taskMan.handle();
    ...
    private UploadFileV2Output completeUploadFileTask(DataTransferStatus status) {
        ...
        CompleteMultipartUploadV2Input input = new CompleteMultipartUploadV2Input().setBucket(this.checkpoint.getBucket()).setKey(this.checkpoint.getKey()).setUploadID(this.checkpoint.getUploadID()).setUploadedParts(uploadedParts).setCallback(this.checkpoint.getCallback()).setCallbackVar(this.checkpoint.getCallbackVar());
        ...
        return new UploadFileV2Output().setRequestInfo(comp.getRequestInfo()).setBucket(comp.getBucket()).setKey(comp.getKey()).setUploadID(checkpoint.getUploadID()).setEtag(comp.getEtag()).setLocation(comp.getLocation()).setHashCrc64ecma(comp.getHashCrc64ecma()).setVersionID(comp.getVersionID()).setSsecAlgorithm(checkpoint.getSseAlgorithm()).setSsecKeyMD5(checkpoint.getSseKeyMd5()).setEncodingType(checkpoint.getEncodingType()).setCallbackResult(comp.getCallbackResult());
    }

    private boolean readyForComplete() {
        if (checkpoint == null || checkpoint.getBucket() == null || checkpoint.getKey() == null || checkpoint.getUploadID() == null || checkpoint.getUploadPartInfos() == null || uploadedParts == null) {
            return false;
        }
        ...
        if (input.getDataTransferListener() != null && this.consumedBytes.get() != checkpoint.getFileSize()) {
            throw new TosClientException("tos: some upload tasks failed, total: " + checkpoint.getFileSize() + ", consumed: " + this.consumedBytes.get(), null);
        }
        return true;
    }
    ...
    private void validateCheckpointPath() {
        String checkpointFileSuffix = Util.checkpointPathMd5(input.getBucket(), input.getKey(), "") + Consts.UPLOAD_CHECKPOINT_FILE_SUFFIX;
    ...
    private void setCheckpoint(UploadFileInfo fileInfo) {
        ...
        boolean valid = false;
        if (checkpoint != null) {
            valid = checkpoint.isValid(fileInfo.getFileSize(), fileInfo.getLastModified(), this.input.getBucket(), this.input.getKey(), input.getFilePath());
            if (!valid) {
                Util.deleteCheckpointFile(input.getCheckpointFile());
            } else {
    ...
    private UploadFileV2Checkpoint initCheckpoint(UploadFileInfo info) throws TosException {
        UploadFileV2Checkpoint checkpoint = new UploadFileV2Checkpoint().setBucket(input.getBucket()).setKey(input.getKey()).setUploadPartInfos(getPartsFromFile(info.getFileSize(), input.getPartSize())).setFilePath(info.getFilePath()).setFileSize(info.getFileSize()).setLastModified(info.getLastModified()).setCallback(input.getCallback()).setCallbackVar(input.getCallbackVar());
        CreateMultipartUploadOutput output;
        UploadEvent createMultipart = new UploadEvent().setBucket(input.getBucket()).setKey(input.getKey()).setCheckpointFile(input.getCheckpointFile()).setFilePath(input.getFilePath());
        try {
            output = this.handler.createMultipartUpload(new CreateMultipartUploadInput().setBucket(input.getBucket()).setKey(input.getKey()).setOptions(input.getOptions()).setEncodingType(input.getEncodingType()));
    ...
    private List<UploadPartInfo> getPartsFromFile(long uploadFileSize, long partSize) {
        // 特殊处理空文件
        if (uploadFileSize == 0) {
            return Collections.singletonList(new UploadPartInfo().setPartNumber(1).setPartSize(0).setOffset(0));
        }
        ...
        for (int i = 0; i < partNum; i++) {
            long currentPartSize = (i == partNum - 1) ? lastPartSize : partSize;
            partInfoList.add(new UploadPartInfo().setPartNumber(i + 1).setPartSize(currentPartSize).setOffset((long) i * partSize));
        }
    ...
    private void combineCrcAndCheck(String serverCrc64Value) {
        if (checkpoint.getUploadPartInfos() == null || checkpoint.getUploadPartInfos().size() == 0 || serverCrc64Value == null) {
            return;
        }
        long crc = getCrc();
